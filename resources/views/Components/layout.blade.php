                ...
                <div class="flex h-16 items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div class="hidden md:block">
                            <div class="ml-10 flex items-baseline space-x-4">
                                ...
                                <x-nav-link href="{{ route('jobs.index') }}" :active="request()->is('jobs')">
                                    Jobs
                                </x-nav-link>
                                <x-nav-link href="{{ route('contact') }}" :active="request()->is('contact')">
                                    Contact
                                </x-nav-link>
                            </div>
                        </div>
                    ...
                    <div class="hidden md:block">
                        <div class="ml-4 flex items-center md:ml-6 space-x-4">
                            @guest
                                <x-nav-link href="{{ route('auth.login') }}" :active="request()->is('auth/login')">
                                    Log In
                                </x-nav-link>
                                <x-nav-link href="{{ route('auth.register') }}" :active="request()->is('auth/register')">
                                    Register
                                </x-nav-link>
                            @endguest
                            @auth
                                <form method="POST" action="{{ route('auth.logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="rounded-md px-3 py-2 text-sm text-gray-300 font-medium hover:bg-gray-700 hover:text-white">
                                        Log Out
                                    </button>
                                </form>
                            @endauth
